...
#    test_data = torchvision.datasets.FashionMNIST(root='./test', train=True, transform=ToTensor(), download=True)
    ...
    encoder=Encoder()
    encoder.load_state_dict(torch.load("models/encoder_model_"+str(aux)+".pkl", map_location=device))
    encoder.to(device)
    decoder=Decoder()
    decoder.load_state_dict(torch.load("models/decoder_model_"+str(aux)+".pkl", map_location=device))
    decoder.to(device)
    ...
    bins=torch.load("data/latent_small_bins_"+str(aux)+".pt", map_location=device)
